#!/usr/bin/env python
import project.clientService as clientService
import project.myService as myService
...
class ServerHandler:
	...
	def setupDb(self):
		try:
			self.cur.execute(
				'CREATE TABLE Users(Id INT, Username TEXT Unique, Password TEXT, Name TEXT, email TEXT Unique, primary key(Id))')
			self.cur.execute(
				'CREATE TABLE Devices(UserId INT, '
				'DeviceId INT,'
				' DeviceName TEXT,'
				' DeviceMac TEXT Unique,'
				' DeviceIp TEXT,'
				' DevicePort TEXT, Status INT, primary key(UserId, DeviceId), foreign key(UserId) references Users(Id) )')
	...
	def login(self, username, password, mac):
		...
		if bcrypt.hashpw(password, ps.encode('utf-8')) == ps:
			...
			query = 'Update Devices Set Status=1 Where DeviceMac="%s"' % macar
			self.cur.execute(query)
		...
		try:  # check if query returned empty. If not empty DeviceExists, else device not exists for that user. Add device.
			...
			while 1:
				...
				try:
					...
					break
			query = ('INSERT INTO Devices VALUES(%s,%s, "%s", "%s", "%s", "%s",1)' % (
			self.userid, generatedId, name, mac, transport.clientaddr[0], transport.clientaddr[1]))
			self.cur.execute(query)
